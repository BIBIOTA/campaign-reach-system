...
- **Docker-pending (carry-over verification item):** run the suite on a Docker-enabled CI runner to actually execute the skipped `@RequiresDocker` tests — `CampaignPersistenceIntegrationTest` (optimistic-lock stale-version failure, audit field recording, 5×status + 3×type enum round-trip) and `CouponPersistenceIntegrationTest` (SHARED_CODE single / UNIQUE_CODE multi-code persistence, duplicate-redemption unique-key block, atomic `used_count` cap rejection). Tracked alongside the §1.3 carry-over.
