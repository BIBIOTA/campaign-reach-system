...
## 1. 依賴與版本目錄
- [x] 1.1 在 `gradle/libs.versions.toml` 加入 springdoc starter 與 swagger-annotations
  ...
  - status: passing
- [x] 1.2 在 `:app` 加入 springdoc starter 依賴；在 `:campaign`、`:reach` 加入 swagger-annotations（`implementation`）
...
## 2. OpenAPI 設定與安全
- [x] 2.1 新增 `OpenApiConfig`（`:app`）提供全域 `OpenAPI` bean
  ...
  - Independence: serial
  - status: passing
- [x] 2.2 新增 `SwaggerDocsSecurityConfig`（`:app`）放行文件路徑
...
## 3. 繁體中文 API 註解
- [x] 3.1 為 `CampaignController` 四個端點加 `@Tag` / `@Operation` / `@ApiResponse`
  ...
  - status: passing
- [x] 3.2 為 `ReachMetricsController` 兩個 GET 端點加 `@Tag` / `@Operation` / `@ApiResponse`
  - Acceptance: WHEN 產生 spec THEN 兩端點有繁中 summary/description AND 標註 404 等錯誤回應
  - Depends on: 1.2
  - Independence: parallel-safe
  - status: passing
- [x] 3.3 為 campaign 與 reach 的 request/response/view DTO 欄位加 `@Schema(description=...)` 繁中說明
...
## 4. Spec 匯出與測試
- [x] 4.1 新增 `OpenApiSpecExportTest`（`:app/src/test`，`@RequiresDocker`）
